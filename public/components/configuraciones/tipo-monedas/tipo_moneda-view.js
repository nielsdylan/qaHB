class TipoMonedaView {
    ...
    listar = () => {
        const $tabla = $('#tabla-data').DataTable({
            ...
            order: [[0, 'asc']],
            ajax: {
                url: route('hb.configuraciones.tipo-monedas.listar'),
                method: 'POST',
    ...
    eventos = () => {
        ...
        $('#nuevo').click((e) => {
            e.preventDefault();
            $('#guardar')[0].reset();
            $('#modal-moneda').find('.modal-title').text('Nueva Moneda')
        ...
        $("#tabla-data").on("click", "button.editar", (e) => {
            ...
            this.model.editar(id).then((respuesta) => {
                ...
                $('#modal-moneda').find('.modal-title').text('Editar Moneda')
                $('#modal-moneda').modal('show');
            }).fail((respuesta) => {
            }).always(() => {
            });
