class LogActividadView {
    ...
    listar = () => {
        const $tabla = $('#tabla-data').DataTable({
            ...
            order: [[0, 'asc']],
            ajax: {
                url: route('hb.configuraciones.log-actividades.listar'),
                method: 'POST',
            ...
            columns: [
                ...
                {data: 'tipo_actividad', className: 'text-center'},
                {data: 'tabla', className: 'text-center'},
    ...
    eventos = () => {
        ...
        $("#tabla-data").on("click", "button.ver", (e) => {
            // $('#guardar')[0].reset();
            // $('#modal-actividades').find('.modal-title').text('Ver Actividades')
            ...
            let id = $(e.currentTarget).attr('data-id'),
                form = $('<form action="'+route('hb.configuraciones.log-actividades.ver')+'" method="POST">'+
